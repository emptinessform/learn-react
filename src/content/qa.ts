...
export const qa: QnAItem[] = [
  ...
  },
  {
    ...
    answer:
      '보통 두 경우입니다. (1) 이펙트 안에서 어떤 state를 바꾸는데 그 state가 다시 이펙트를 실행시키는 순환 — 의존성을 지우지 말고, 매번 바꾸지 않도록 조건을 두거나 로직을 분리하세요. (2) 객체·함수처럼 매 렌더마다 새로 만들어지는 값을 의존성에 넣은 경우 — useMemo/useCallback으로 참조를 유지하거나 컴포넌트 밖으로 옮기세요. 의존성 배열은 "비우는" 게 아니라 "정확히 채우는" 것이 원칙입니다.',
    date: '2026-06-28',
